...
class DeviceManager:
    ...
    def get_device_config() -> Dict[str, Any]:
        ...
        if torch.cuda.is_available():
            ...
            logger.info(f"✓ Using CUDA: {torch.cuda.get_device_name(0)} (FP16)")
...
class ModelWrapper:
    ...
    def __init__(self, model_id: str, device_config: Dict[str, Any], config: SummaryConfig):
        self.config = config
...
class LegalSummarizer:
    ...
    def __init__(self, model_type: str, config: Optional[SummaryConfig] = None):
        ...
        if self.model_type == 'led':
            self.model = LEDSummarizer(LEDSummarizer.MODEL_PATH, device_config, self.config)
        elif self.model_type == 'pegasus':
            self.model = PegasusSummarizer(PegasusSummarizer.MODEL_PATH, device_config, self.config)
        elif self.model_type == 'hybrid':
            self.led = LEDSummarizer(LEDSummarizer.MODEL_PATH, device_config, self.config)
            self.pegasus = PegasusSummarizer(PegasusSummarizer.MODEL_PATH, device_config, self.config)
